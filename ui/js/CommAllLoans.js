    ...
    loadLoans(loansArray) {
      ...
      loansArray.forEach((element, index) => {
        ...
        if (element.loan_status === 'new') {
          ...
          cellRequest.innerHTML = 'Committee Accepted';
        }
    ...
    requestEventclicks() {
      ...
      for (let i = 0; i < acceptBtn.length; i += 1) {
        acceptBtn[i].onclick = () => {
          ...
          const payload = {
            loanStatus: 'committee-accepted'
          };
          this.updateOrderStatus(loanId, payload)
            .then((res) => {
              if (res.loan.loan_status === 'committee-accepted') {
                acceptBtn[i].parentElement.innerHTML = `<div>accepted</div>`;
                this.requestEventclicks();
              }
            });
        };
      ...
      for (let i = 0; i < rejectBtn.length; i += 1) {
        rejectBtn[i].onclick = () => {
          ...
          selectYes[i].onclick = () => {
            const payload = {
              loanStatus: 'committee-cancelled'
            };
            this.updateOrderStatus(loanId, payload)
              .then((res) => {
                if (res.loan.loan_status === 'committee-cancelled') {
